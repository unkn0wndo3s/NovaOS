...
static void *memset(void *s, int c, size_t n){ uint8_t*p=(uint8_t*)s; for(size_t i=0;i<n;i++) p[i]=(uint8_t)c; return s; }
...
static void blit_center(struct limine_framebuffer *fb, const uint8_t *rgba, int w, int h){
    ...
    for (int y=0; y<h; y++){
        ...
        for (int x=0; x<w; x++){
            ...
            put_px(fb,(uint64_t)px,(uint64_t)py, fb_pack(fb,R,G,B));
        }
    }
}
...
// -------- Limine modules helper --------
typedef struct { const uint8_t *data; size_t size; } file_t;
static file_t find_module_by_suffix(const char *suffix){
    const struct limine_module_response *resp = mod_req.response;
    if (!resp) return (file_t){0,0};
    size_t sl=0; while(suffix[sl]) sl++;
    for (uint64_t i=0; i<resp->module_count; i++){
        struct limine_file *f = resp->modules[i]; if (!f || !f->path) continue;
        const char *p=f->path; size_t pl=0; while(p[pl]) pl++;
        size_t a=sl,b=pl; while(a&&b&&suffix[a-1]==p[b-1]){a--;b--;}
        if (a==0) return (file_t){ (const uint8_t*)f->address, (size_t)f->size };
    }
    return (file_t){0,0};
}
...
static void play_once(struct limine_framebuffer *fb, const gif_t *g){
    ...
    for(i=0;i<g->count;i++){
        ...
        { int d = g->delays_ms[i]>0? g->delays_ms[i] : 33; spin_ms((uint32_t)d); }
    }
}
static void play_loop(struct limine_framebuffer *fb, const gif_t *g, uint32_t min_ms){
    ...
    while(t<min_ms){
        const uint8_t *fr = g->frames + (size_t)g->w*(size_t)g->h*4u*(size_t)i;
        blit_center(fb, fr, g->w, g->h);
        { int d = g->delays_ms[i]>0? g->delays_ms[i] : 33; spin_ms((uint32_t)d); t += (uint32_t)d; }
        i = (i+1) % g->count;
    }
}
...
void _start(void){
    ...
    file_t m1 = find_module_by_suffix("/loader/stage1.gif");
    file_t m2 = find_module_by_suffix("/loader/stage2.gif");
    file_t m3 = find_module_by_suffix("/loader/stage3.gif");

    gif_t g1={0}, g2={0}, g3={0};
    if(m1.data) gif_decode(m1.data, m1.size, &g1);
    if(m2.data) gif_decode(m2.data, m2.size, &g2);
    if(m3.data) gif_decode(m3.data, m3.size, &g3);

    if(g1.frames) play_once(fb, &g1);            // Stage 1: une fois
    if(g2.frames){                                // Stage 2: boucle pendant l'init (min 3.5s)
    ...
    if(g3.frames) play_once(fb, &g3);            // Stage 3: une fois
